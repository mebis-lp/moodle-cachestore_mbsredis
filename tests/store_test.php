...
class store_test extends \cachestore_tests {
     ...
     * @return cachestore_mbsredis
     */
    protected function create_cachestore_mbsredis(array $extraconfig = [], bool $ttl = false): cachestore_mbsredis {
        if ($ttl) {
        ...
        $configuration = array_merge(cachestore_mbsredis::unit_test_configuration(), $extraconfig);
        $store = new cachestore_mbsredis('Test', $configuration);
    ...
    public function test_lock_timeouts(): void {
        $store = $this->create_cachestore_mbsredis(['lockwait' => 2, 'locktimeout' => 4]);
    ...
    public function test_get_last_io_bytes_compressed(): void {
        $store = $this->create_cachestore_mbsredis(['compressor' => cachestore_mbsredis::COMPRESSOR_PHP_GZIP]);
    ...
    public function test_delete_many(bool $ttl): void {
        $store = $this->create_cachestore_mbsredis([], $ttl);
